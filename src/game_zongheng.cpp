...
const ZH_Adjutant* get_adjutant_by_id(int id) {
    if(id <= 0) return NULL;
    for(int i=0; i<zh_data_adjutants_count; i++) if(zh_data_adjutants[i].id == id) return &zh_data_adjutants[i];
    return NULL;
}
...
static bool craft_item(int recipe_idx) {
    ...
    if (zh_player.gold < r.cost_gold) { zh_log("锻造失败：铜贝不足以支付手工费！"); return false; }
    ...
    int c1 = 0, c2 = 0;
    for(int i=0; i<50; i++) {
        if (c1 < r.mat1_count && zh_player.inventory[i] == r.mat1_id) { zh_player.inventory[i] = -1; c1++; continue; }
        if (r.mat2_id != -1 && c2 < r.mat2_count && zh_player.inventory[i] == r.mat2_id) { zh_player.inventory[i] = -1; c2++; }
    }
...
static void open_forge_ui() {
    if (!modal_forge) {
        ...
        lv_obj_add_style(l_c, &style_cn, 0); lv_label_set_text(l_c, "离开火炉"); lv_obj_center(l_c);
...
void open_adjutant_roster() {
    ...
    if(!has_adj) {
        lv_obj_t * lbl_none = lv_label_create(list_adj_roster); lv_obj_add_style(lbl_none, &style_cn, 0);
        lv_obj_set_style_text_color(lbl_none, lv_color_hex(0xFFFFFF), 0);
        lv_label_set_text(lbl_none, "\n您麾下没有该类型的副官。"); lv_obj_center(lbl_none);
    }
}
...
void open_skill_selector() {
    ...
    if(!has_skill) {
        lv_obj_t * lbl_none = lv_label_create(list_learned_skills); lv_obj_add_style(lbl_none, &style_cn, 0);
        lv_obj_set_style_text_color(lbl_none, lv_color_hex(0xFFFFFF), 0);
        lv_label_set_text(lbl_none, "\n您还未学会此类技能"); lv_obj_center(lbl_none);
    }
...
void check_levelup() {
    ...
    while (zh_player.exp >= need_exp) {
        ...
        char buf[128]; snprintf(buf, sizeof(buf), "升级了！当前等级: %d\n生命、魔法、攻击、防御全面提升！", zh_player.level);
        zh_log(buf); need_exp = zh_player.level * 100; 
    }
}
...
void player_die() {
    zh_player.gold = 0; zh_player.hp = zh_player.max_hp / 2; zh_player.mp = zh_player.max_mp / 2;
    if (zh_player.crime_value > 0) {
        zh_player.crime_value = 0; zh_log("被打败，铜贝掉落！\n幸被送往教堂/医馆，罪恶清零。");
    } else zh_log("被打败！铜贝被洗劫一空！\n你被抬到了最近的教堂/医馆。");
    ...
    for(int i = 0; i < zh_data_locations_count; i++) {
        if(zh_data_locations[i].id >= base_id && zh_data_locations[i].id < base_id + 40 && zh_data_locations[i].action_type == 6) { 
            target_church = zh_data_locations[i].id; break; 
        }
    }
...
void spawn_monsters_for_loc(int loc_id) {
    ...
    for(int i=0; i<5; i++) {
        ...
        if(mid != -1 && zh_data_monsters[mid].is_aggressive && rand() % 100 < 30) {
            ...
            zh_log(buf); start_turn_based_combat(i); break; 
        }
    }
}

void execute_combat(int monster_idx) { if (zh_player.current_monsters[monster_idx] == -1) return; start_turn_based_combat(monster_idx); }

void populate_map_list() {
    lv_obj_clean(list_map_locs);
    int base = (zh_player.location_id / 40) * 40;
    for(int i=0; i<zh_data_locations_count; i++) {
        if(zh_data_locations[i].id >= base && zh_data_locations[i].id < base+40 && zh_data_locations[i].action_type != 1 && zh_data_locations[i].name[0] != '\0') {
            lv_obj_t * btn = lv_list_add_btn(list_map_locs, LV_SYMBOL_RIGHT, zh_data_locations[i].name);
            lv_obj_set_style_bg_color(btn, lv_color_hex(0x34495e), 0);
            lv_obj_t * lbl = lv_obj_get_child(btn, 1); if(lbl){ lv_obj_add_style(lbl, &style_cn, 0); lv_obj_set_style_text_color(lbl, lv_color_hex(0xFFFFFF), 0); }
            lv_obj_add_event_cb(btn, [](lv_event_t *e){
...
void start_sailing(int origin_id, int target_id) {
    ...
    lv_label_set_text(lbl_sailing_info, msg); lv_obj_clear_flag(modal_sailing, LV_OBJ_FLAG_HIDDEN);
}
...
void refresh_zongheng_ui() {
    ...
    auto get_eq_name = [](int id) -> const char* { if(id == -1) return "无"; return get_item_by_id(id).name; };
    ...
    for (int i = 0; i < 4; i++) {
        if (loc->links[i] != -1) {
            int tid = loc->links[i]; const char* tname = "未知";
            for(int j = 0; j < zh_data_locations_count; j++) if(zh_data_locations[j].id == tid) tname = zh_data_locations[j].name;
            lv_obj_t * btn = lv_list_add_btn(list_zh_move, LV_SYMBOL_RIGHT, tname);
            lv_obj_set_style_bg_color(btn, lv_color_hex(0x2c3e50), 0); lv_obj_set_style_border_width(btn, 0, 0); 
            lv_obj_t * lbl = lv_obj_get_child(btn, 1); if(lbl){ lv_obj_add_style(lbl, &style_cn, 0); lv_obj_set_style_text_color(lbl, lv_color_hex(0xFFFFFF), 0); }
            ...
            lv_obj_add_event_cb(btn, [](lv_event_t *e){
                ...
                zh_market_state = 0; 
                if (is_cross_port && zh_player.crime_value >= 100) { zh_log("【警告】通缉犯禁止出港！去警局处理！"); return; }
                if (is_cross_port) { start_sailing(zh_player.location_id, tid); return; } 
    ...
    auto add_act_btn = [](const char* txt, lv_event_cb_t cb, uint32_t color = 0x2c3e50, void* user_data = NULL) {
        ...
        lv_obj_set_style_bg_color(btn, lv_color_hex(color), 0); lv_obj_set_style_border_width(btn, 0, 0);
        lv_obj_t * lbl = lv_obj_get_child(btn, 1); if(lbl){ lv_obj_add_style(lbl, &style_cn, 0); lv_obj_set_style_text_color(lbl, lv_color_hex(0xFFFFFF), 0); }
    ...
    if (loc->action_type == 1 || loc->action_type == 13) { 
        ...
        for(int i=0; i<5; i++) {
            int mid = zh_player.current_monsters[i];
            if (mid != -1) {
                has_monster = true; const ZH_Monster* m = &zh_data_monsters[mid]; int hp = zh_player.current_monsters_hp[i];
                char buf[64]; snprintf(buf, sizeof(buf), "%s %s (Lv.%d | HP:%d)", m->is_aggressive ? "[主动]" : "攻击", m->name, m->level, hp);
    ...
    if (loc->action_type == 14 || loc->action_type == 0 || loc->action_type == 13) {
        add_act_btn("招募水手/副官 (花费 1 银贝)", [](lv_event_t *e){ 
            if(zh_player.silver >= 1) {
                int empty_slot = -1;
                for(int i=0; i<10; i++) { if(zh_player.adjutants_roster[i] == 0) { empty_slot = i; break; } }
    ...
    auto open_npc_cb = [](lv_event_t *e) {
        ...
        auto add_n_btn = [](const char* txt, lv_event_cb_t cb, uint32_t color) {
            ...
            lv_obj_t * l = lv_label_create(b); lv_obj_add_style(l, &style_cn, 0); lv_label_set_text(l, txt); lv_obj_center(l);
        ...
        add_n_btn("继续对话", [](lv_event_t *e){
            ...
            if (is_injured) zh_log("对方捂着伤口，恐惧地看着你不敢说话。");
                ...
                if(npc->gift_item_id != -1 && !has_gift) {
                    bool added = false;
                    for(int k=0; k<50; k++) { if(zh_player.inventory[k] == -1) { zh_player.inventory[k] = npc->gift_item_id; added = true; break; } }
                    if(added) { zh_player.npc_gift_bits[word_idx] |= (1 << bit_idx); char buf[128]; snprintf(buf, sizeof(buf), "【意外之喜】\n%s 塞给你一件物品！", npc->name); zh_log(buf); }
    ...
    if (lv_obj_get_child_cnt(list_zh_action) == 0) {
        lv_obj_t * l = lv_label_create(list_zh_action); lv_obj_add_style(l, &style_cn, 0); lv_obj_set_style_text_color(l, lv_color_hex(0xFFFFFF), 0); lv_label_set_text(l, "这里没什么可操作的..."); lv_obj_center(l);
    }

    lv_obj_clean(list_zh_bag);
    lv_obj_t * filter_cont = lv_obj_create(list_zh_bag); lv_obj_set_size(filter_cont, 220, 40); lv_obj_set_style_bg_color(filter_cont, lv_color_hex(0x1a252f), 0); lv_obj_set_style_border_width(filter_cont, 0, 0); lv_obj_set_style_pad_all(filter_cont, 0, 0); lv_obj_clear_flag(filter_cont, LV_OBJ_FLAG_SCROLLABLE);
    ...
    for (int i = 0; i < 50; i++) {
        if (zh_player.inventory[i] != -1) {
            ...
            if (show) {
                has_item = true; const char* icon = LV_SYMBOL_DUMMY; uint32_t color = 0x2c3e50; 
                ...
                lv_obj_t * btn = lv_list_add_btn(list_zh_bag, icon, item.name); lv_obj_set_style_bg_color(btn, lv_color_hex(color), 0);
                lv_obj_t * lbl = lv_obj_get_child(btn, 1); if(lbl){ lv_obj_add_style(lbl, &style_cn, 0); lv_obj_set_style_text_color(lbl, lv_color_hex(0xFFFFFF), 0); }
                lv_obj_add_event_cb(btn, [](lv_event_t *e){
                    ...
                    if (i_ptr.type >= 1 && i_ptr.type <= 5) { const char* slots[] = {"武器", "头盔", "胸甲", "护腿", "鞋子"}; snprintf(desc_buf, sizeof(desc_buf), "部位: %s\n属性值: +%d\n售价: %d 铜贝\n\n点击装备替换当前部位。", slots[i_ptr.type-1], i_ptr.value, i_ptr.price); } 
                        ...
                        if (i_ptr.id == 201 || i_ptr.id == 206 || i_ptr.id == 216 || i_ptr.id == 217 || i_ptr.id == 237 || i_ptr.id == 241 || i_ptr.id == 249 || i_ptr.id == 250) { snprintf(desc_buf, sizeof(desc_buf), "【消耗型异宝】\n售价: %d 铜贝\n\n点击[使用]释放力量，改变命运！", i_ptr.price); } 
                        else if (i_ptr.id >= 401 && i_ptr.id <= 410) snprintf(desc_buf, sizeof(desc_buf), "【原矿石】\n售价: %d 铜贝\n\n可用于铁匠铺锻造绝世神兵。", i_ptr.price);
                        else if (i_ptr.id >= 421 && i_ptr.id <= 445) snprintf(desc_buf, sizeof(desc_buf), "【锻造材料】\n售价: %d 铜贝\n\n珍稀的野兽素材，锻造神装必备。", i_ptr.price);
                        else snprintf(desc_buf, sizeof(desc_buf), "【被动/任务异宝】\n售价: %d 铜贝\n\n带在包里有惊人的被动加成或用于交任务。", i_ptr.price);
                    } else snprintf(desc_buf, sizeof(desc_buf), "特殊道具\n价值连城。");
                    lv_label_set_text(lbl_item_detail_desc, desc_buf); lv_obj_clear_flag(modal_item_detail, LV_OBJ_FLAG_HIDDEN);
                }, LV_EVENT_CLICKED, (void*)(intptr_t)i);
            }
        }
    }
    if (!has_item) { lv_obj_t * l = lv_label_create(list_zh_bag); lv_obj_add_style(l, &style_cn, 0); lv_obj_set_style_text_color(l, lv_color_hex(0x95a5a6), 0); lv_label_set_text(l, "\n没有符合条件的物品"); lv_obj_center(l); }
}
...
// 创建并验证主场景对象
    ...
    lv_obj_t * btn_menu = lv_btn_create(scr_zongheng); lv_obj_set_size(btn_menu, 50, 30); lv_obj_align(btn_menu, LV_ALIGN_TOP_RIGHT, -5, 5);
    lv_obj_set_style_bg_color(btn_menu, lv_color_hex(0x34495e), 0); lv_obj_t * lbl_menu = lv_label_create(btn_menu);
    lv_obj_add_style(lbl_menu, &style_cn, 0); lv_label_set_text(lbl_menu, "菜单"); lv_obj_center(lbl_menu);
    lv_obj_add_event_cb(btn_menu, [](lv_event_t *e){ lv_obj_clear_flag(modal_zh_menu, LV_OBJ_FLAG_HIDDEN); }, LV_EVENT_CLICKED, NULL);

    lv_obj_t * btn_map = lv_btn_create(scr_zongheng); lv_obj_set_size(btn_map, 50, 30); lv_obj_align(btn_map, LV_ALIGN_TOP_RIGHT, -60, 5);
    lv_obj_set_style_bg_color(btn_map, lv_color_hex(0x2980b9), 0); lv_obj_t * lbl_map = lv_label_create(btn_map);
    lv_obj_add_style(lbl_map, &style_cn, 0); lv_label_set_text(lbl_map, "地图"); lv_obj_center(lbl_map);
    lv_obj_add_event_cb(btn_map, [](lv_event_t *e){ populate_map_list(); lv_obj_clear_flag(modal_zh_map, LV_OBJ_FLAG_HIDDEN); }, LV_EVENT_CLICKED, NULL);

    lv_obj_t * log_cont = lv_obj_create(scr_zongheng); lv_obj_set_size(log_cont, 230, 110); lv_obj_align(log_cont, LV_ALIGN_TOP_MID, 0, 45); 
    lv_obj_set_style_bg_color(log_cont, lv_color_hex(0x2c3e50), 0); lv_obj_set_style_border_width(log_cont, 0, 0);
    lbl_zh_log = lv_label_create(log_cont); lv_obj_set_width(lbl_zh_log, 210); lv_obj_add_style(lbl_zh_log, &style_cn, 0);
    lv_obj_set_style_text_color(lbl_zh_log, lv_color_hex(0xecf0f1), 0); lv_label_set_text(lbl_zh_log, "欢迎来到大航海时代！");

    lv_obj_t * tv = lv_tabview_create(scr_zongheng, LV_DIR_TOP, 35); lv_obj_set_size(tv, 240, 165); lv_obj_align(tv, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_t * tv_cont = lv_tabview_get_content(tv); lv_obj_set_style_bg_color(tv_cont, lv_color_hex(0x1a252f), 0); lv_obj_set_style_bg_opa(tv_cont, LV_OPA_COVER, 0);
    lv_obj_t * tv_btns = lv_tabview_get_tab_btns(tv); lv_obj_add_style(tv_btns, &style_cn, 0); 
    lv_obj_set_style_bg_color(tv_btns, lv_color_hex(0x222222), 0); lv_obj_set_style_text_color(tv_btns, lv_color_hex(0xCCCCCC), 0); lv_obj_set_style_pad_left(tv_btns, 0, 0); lv_obj_set_style_pad_right(tv_btns, 0, 0);

    tab_zh_move = lv_tabview_add_tab(tv, "移动"); tab_zh_action = lv_tabview_add_tab(tv, "行动"); tab_zh_status = lv_tabview_add_tab(tv, "状态"); tab_zh_bag = lv_tabview_add_tab(tv, "背包");
    tab_zh_quest = lv_tabview_add_tab(tv, "任务");

    list_zh_move = lv_list_create(tab_zh_move); lv_obj_set_size(list_zh_move, 240, 130); lv_obj_align(list_zh_move, LV_ALIGN_TOP_LEFT, -15, -15); lv_obj_set_style_bg_color(list_zh_move, lv_color_hex(0x1a252f), 0); lv_obj_set_style_border_width(list_zh_move, 0, 0); 
    list_zh_action = lv_list_create(tab_zh_action); lv_obj_set_size(list_zh_action, 240, 130); lv_obj_align(list_zh_action, LV_ALIGN_TOP_LEFT, -15, -15); lv_obj_set_style_bg_color(list_zh_action, lv_color_hex(0x1a252f), 0); lv_obj_set_style_border_width(list_zh_action, 0, 0); 
    list_zh_bag = lv_list_create(tab_zh_bag); lv_obj_set_size(list_zh_bag, 240, 130); lv_obj_align(list_zh_bag, LV_ALIGN_TOP_LEFT, -15, -15); lv_obj_set_style_bg_color(list_zh_bag, lv_color_hex(0x1a252f), 0); lv_obj_set_style_border_width(list_zh_bag, 0, 0); 

    lv_obj_set_style_bg_color(tab_zh_status, lv_color_hex(0x1a252f), 0); lv_obj_set_style_border_width(tab_zh_status, 0, 0); lv_obj_set_style_pad_all(tab_zh_status, 10, 0);
    lbl_zh_detail_status = lv_label_create(tab_zh_status); lv_obj_add_style(lbl_zh_detail_status, &style_cn, 0); lv_obj_set_style_text_color(lbl_zh_detail_status, lv_color_hex(0xecf0f1), 0);

    modal_zh_menu = lv_obj_create(scr_zongheng); lv_obj_set_size(modal_zh_menu, 180, 270); lv_obj_center(modal_zh_menu);
    lv_obj_set_style_bg_color(modal_zh_menu, lv_color_hex(0x111111), 0); lv_obj_add_flag(modal_zh_menu, LV_OBJ_FLAG_HIDDEN);
    const char* m_opts[] = {"继续游戏", "配置技能", "副官任命", "保存进度", "重新开始", "退出游戏"};
    for(int i=0; i<6; i++) {
        lv_obj_t * b = lv_btn_create(modal_zh_menu); lv_obj_set_size(b, 150, 36); lv_obj_align(b, LV_ALIGN_TOP_MID, 0, i*40); lv_obj_set_style_bg_color(b, lv_color_hex(0x34495e), 0);
        lv_obj_t * l = lv_label_create(b); lv_obj_add_style(l, &style_cn, 0); lv_label_set_text(l, m_opts[i]); lv_obj_center(l);
    ...
    modal_zh_skill_cfg = lv_obj_create(scr_zongheng); lv_obj_set_size(modal_zh_skill_cfg, 220, 280); lv_obj_center(modal_zh_skill_cfg);
    lv_obj_add_flag(modal_zh_skill_cfg, LV_OBJ_FLAG_HIDDEN); lv_obj_set_style_bg_color(modal_zh_skill_cfg, lv_color_hex(0x2c3e50), 0);
    lv_obj_t * lbl_sc_title = lv_label_create(modal_zh_skill_cfg); lv_obj_add_style(lbl_sc_title, &style_cn, 0); lv_label_set_text(lbl_sc_title, "自定义技能装配"); lv_obj_align(lbl_sc_title, LV_ALIGN_TOP_MID, 0, 0);
    list_skill_slots = lv_list_create(modal_zh_skill_cfg); lv_obj_set_size(list_skill_slots, 200, 200); lv_obj_align(list_skill_slots, LV_ALIGN_TOP_MID, 0, 25);
    lv_obj_set_style_bg_color(list_skill_slots, lv_color_hex(0x1a252f), 0); lv_obj_set_style_border_width(list_skill_slots, 0, 0);
    lv_obj_t * btn_sc_close = lv_btn_create(modal_zh_skill_cfg); lv_obj_set_size(btn_sc_close, 100, 35); lv_obj_align(btn_sc_close, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_bg_color(btn_sc_close, lv_color_hex(0xc0392b), 0); lv_obj_t * lbl_sc_close = lv_label_create(btn_sc_close); lv_obj_add_style(lbl_sc_close, &style_cn, 0); lv_label_set_text(lbl_sc_close, "完成装配"); lv_obj_center(lbl_sc_close);
    ...
    modal_zh_skill_select = lv_obj_create(scr_zongheng); lv_obj_set_size(modal_zh_skill_select, 200, 260); lv_obj_center(modal_zh_skill_select);
    lv_obj_add_flag(modal_zh_skill_select, LV_OBJ_FLAG_HIDDEN); lv_obj_set_style_bg_color(modal_zh_skill_select, lv_color_hex(0x1a252f), 0);
    lv_obj_t * lbl_ss_title = lv_label_create(modal_zh_skill_select); lv_obj_add_style(lbl_ss_title, &style_cn, 0); lv_label_set_text(lbl_ss_title, "选择要装配的技能"); lv_obj_align(lbl_ss_title, LV_ALIGN_TOP_MID, 0, 0);
    list_learned_skills = lv_list_create(modal_zh_skill_select); lv_obj_set_size(list_learned_skills, 180, 190); lv_obj_align(list_learned_skills, LV_ALIGN_TOP_MID, 0, 25);
    lv_obj_set_style_bg_color(list_learned_skills, lv_color_hex(0x2c3e50), 0); lv_obj_set_style_border_width(list_learned_skills, 0, 0);
    lv_obj_t * btn_ss_close = lv_btn_create(modal_zh_skill_select); lv_obj_set_size(btn_ss_close, 80, 30); lv_obj_align(btn_ss_close, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_bg_color(btn_ss_close, lv_color_hex(0x555555), 0); lv_obj_t * lbl_ss_close = lv_label_create(btn_ss_close); lv_obj_add_style(lbl_ss_close, &style_cn, 0); lv_label_set_text(lbl_ss_close, "返回"); lv_obj_center(lbl_ss_close);
    ...
    modal_zh_map = lv_obj_create(scr_zongheng); lv_obj_set_size(modal_zh_map, 220, 280); lv_obj_center(modal_zh_map);
    lv_obj_add_flag(modal_zh_map, LV_OBJ_FLAG_HIDDEN); lv_obj_set_style_bg_color(modal_zh_map, lv_color_hex(0x2c3e50), 0);
    lv_obj_t * map_title = lv_label_create(modal_zh_map); lv_obj_add_style(map_title, &style_cn, 0); lv_label_set_text(map_title, "本港快速传送"); lv_obj_align(map_title, LV_ALIGN_TOP_MID, 0, 0);
    list_map_locs = lv_list_create(modal_zh_map); lv_obj_set_size(list_map_locs, 200, 190); lv_obj_align(list_map_locs, LV_ALIGN_TOP_MID, 0, 25);
    lv_obj_set_style_bg_color(list_map_locs, lv_color_hex(0x1a252f), 0); lv_obj_set_style_border_width(list_map_locs, 0, 0);
    lv_obj_t * btn_map_close = lv_btn_create(modal_zh_map); lv_obj_set_size(btn_map_close, 100, 35); lv_obj_align(btn_map_close, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_bg_color(btn_map_close, lv_color_hex(0x552222), 0); lv_obj_t * lbl_map_close = lv_label_create(btn_map_close); lv_obj_add_style(lbl_map_close, &style_cn, 0); lv_label_set_text(lbl_map_close, "关闭"); lv_obj_center(lbl_map_close); lv_obj_add_event_cb(btn_map_close, [](lv_event_t *e){ lv_obj_add_flag(modal_zh_map, LV_OBJ_FLAG_HIDDEN); }, LV_EVENT_CLICKED, NULL);

    modal_sailing = lv_obj_create(scr_zongheng); lv_obj_set_size(modal_sailing, 220, 240); lv_obj_center(modal_sailing);
    lv_obj_add_flag(modal_sailing, LV_OBJ_FLAG_HIDDEN); lv_obj_set_style_bg_color(modal_sailing, lv_color_hex(0x2c3e50), 0);
    ...
    lbl_item_detail_desc = lv_label_create(modal_item_detail); lv_obj_add_style(lbl_item_detail_desc, &style_cn, 0); lv_obj_set_style_text_color(lbl_item_detail_desc, lv_color_hex(0xecf0f1), 0); lv_obj_align(lbl_item_detail_desc, LV_ALIGN_TOP_LEFT, 0, 25); lv_obj_set_width(lbl_item_detail_desc, 180);
    lv_obj_t * btn_item_use = lv_btn_create(modal_item_detail); lv_obj_set_size(btn_item_use, 180, 35); lv_obj_align(btn_item_use, LV_ALIGN_BOTTOM_MID, 0, -45); lv_obj_set_style_bg_color(btn_item_use, lv_color_hex(0x27ae60), 0);
    lv_obj_t * lbl_iuse = lv_label_create(btn_item_use); lv_obj_add_style(lbl_iuse, &style_cn, 0); lv_label_set_text(lbl_iuse, "使用 / 装备"); lv_obj_center(lbl_iuse);
    lv_obj_add_event_cb(btn_item_use, [](lv_event_t *e){
        lv_obj_add_flag(modal_item_detail, LV_OBJ_FLAG_HIDDEN); if(selected_inventory_idx == -1) return;
        ZH_Item i_ptr = get_item_by_id(zh_player.inventory[selected_inventory_idx]);
        char buf[128]; int* target_slot = nullptr; const char* slot_n = "";
        
        if (i_ptr.type == 1) { target_slot = &zh_player.eq_weapon; slot_n = "武器"; } else if (i_ptr.type == 2) { target_slot = &zh_player.eq_head; slot_n = "头盔"; } else if (i_ptr.type == 3) { target_slot = &zh_player.eq_chest; slot_n = "胸甲"; } else if (i_ptr.type == 4) { target_slot = &zh_player.eq_legs; slot_n = "护腿"; } else if (i_ptr.type == 5) { target_slot = &zh_player.eq_shoes; slot_n = "鞋子"; }
    ...
    modal_npc = lv_obj_create(scr_zongheng); lv_obj_set_size(modal_npc, 200, 260); lv_obj_center(modal_npc);
    lv_obj_add_flag(modal_npc, LV_OBJ_FLAG_HIDDEN); lv_obj_set_style_bg_color(modal_npc, lv_color_hex(0x2c3e50), 0);
    lbl_npc_name = lv_label_create(modal_npc); lv_obj_add_style(lbl_npc_name, &style_cn, 0); lv_obj_set_style_text_color(lbl_npc_name, lv_color_hex(0xF1C40F), 0); lv_obj_align(lbl_npc_name, LV_ALIGN_TOP_MID, 0, -10);
    lbl_npc_dialogue = lv_label_create(modal_npc); lv_obj_add_style(lbl_npc_dialogue, &style_cn, 0); lv_obj_set_style_text_color(lbl_npc_dialogue, lv_color_hex(0xecf0f1), 0); lv_obj_set_width(lbl_npc_dialogue, 180); lv_obj_align(lbl_npc_dialogue, LV_ALIGN_TOP_LEFT, 0, 15);
    cont_npc_actions = lv_obj_create(modal_npc); lv_obj_set_size(cont_npc_actions, 190, 150); lv_obj_align(cont_npc_actions, LV_ALIGN_BOTTOM_MID, 0, 10);
    lv_obj_set_style_bg_opa(cont_npc_actions, LV_OPA_TRANSP, 0); lv_obj_set_style_border_width(cont_npc_actions, 0, 0); lv_obj_set_style_pad_all(cont_npc_actions, 0, 0);
    lv_obj_set_flex_flow(cont_npc_actions, LV_FLEX_FLOW_COLUMN); lv_obj_set_flex_align(cont_npc_actions, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER); lv_obj_set_style_pad_row(cont_npc_actions, 5, 0);

    modal_adjutant = lv_obj_create(scr_zongheng); lv_obj_set_size(modal_adjutant, 220, 280); lv_obj_center(modal_adjutant);
    lv_obj_add_flag(modal_adjutant, LV_OBJ_FLAG_HIDDEN); lv_obj_set_style_bg_color(modal_adjutant, lv_color_hex(0x2c3e50), 0);
    lv_obj_t * lbl_adj_title = lv_label_create(modal_adjutant); lv_obj_add_style(lbl_adj_title, &style_cn, 0); lv_label_set_text(lbl_adj_title, "舰队副官管理"); lv_obj_align(lbl_adj_title, LV_ALIGN_TOP_MID, 0, 0);
    list_adj_slots = lv_list_create(modal_adjutant); lv_obj_set_size(list_adj_slots, 200, 200); lv_obj_align(list_adj_slots, LV_ALIGN_TOP_MID, 0, 25); lv_obj_set_style_bg_color(list_adj_slots, lv_color_hex(0x1a252f), 0);
    list_adj_roster = lv_list_create(modal_adjutant); lv_obj_set_size(list_adj_roster, 200, 200); lv_obj_align(list_adj_roster, LV_ALIGN_TOP_MID, 0, 25); lv_obj_set_style_bg_color(list_adj_roster, lv_color_hex(0x1a252f), 0); lv_obj_add_flag(list_adj_roster, LV_OBJ_FLAG_HIDDEN); 
    lv_obj_t * btn_adj_close = lv_btn_create(modal_adjutant); lv_obj_set_size(btn_adj_close, 100, 35); lv_obj_align(btn_adj_close, LV_ALIGN_BOTTOM_MID, 0, 0); lv_obj_set_style_bg_color(btn_adj_close, lv_color_hex(0xc0392b), 0); 
    lv_obj_t * lbl_adj_close = lv_label_create(btn_adj_close); lv_obj_add_style(lbl_adj_close, &style_cn, 0); lv_label_set_text(lbl_adj_close, "关闭"); lv_obj_center(lbl_adj_close);
    lv_obj_add_event_cb(btn_adj_close, [](lv_event_t *e){ lv_obj_add_flag(modal_adjutant, LV_OBJ_FLAG_HIDDEN); lv_obj_add_flag(list_adj_roster, LV_OBJ_FLAG_HIDDEN); lv_obj_clear_flag(list_adj_slots, LV_OBJ_FLAG_HIDDEN); }, LV_EVENT_CLICKED, NULL);
...
bool load_zongheng_game() { 
    ...
    File file = LittleFS.open("/zh_save.dat", FILE_READ); if (!file) return false; 
    for(int i=0; i<4; i++) zh_player.npc_gift_bits[i] = 0; for(int i=0; i<128; i++) zh_player.npc_status[i] = 0;
    ...
    file.read((uint8_t*)&zh_player, file.size() < sizeof(zh_player) ? file.size() : sizeof(zh_player)); file.close(); 
    ...
    for(int i=0; i<5; i++) { if(zh_player.current_monsters[i] < -1 || zh_player.current_monsters[i] >= zh_data_monsters_count) { needs_monster_reset = true; break; } }
